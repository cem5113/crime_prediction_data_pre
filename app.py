...
os.environ["DEMOG_WHITELIST"] = st.secrets.get(
    "DEMOG_WHITELIST", os.environ.get("DEMOG_WHITELIST", "")
)
...
def _group_rare_labels(
    ...
    """
    col içindeki nadir etiketleri Other altında toplar. Öncelik: min_prop (oran) -> min_count (mutlak).
    """
    ...
    try:
        ...
        if "category" in df.columns:
            df["category_grouped"] = _group_rare_labels(
                df,
                "category",  # Env ile override edilebilir; yoksa varsayılan %1 veya 200
                min_prop=None,
                min_count=None,
                other_label="Other",
    ...
    try:
        if {"date","GEOID"}.issubset(df.columns):
            ...
            cat_col = "category_grouped" if "category_grouped" in df.columns else (
                "subcategory_grouped" if "subcategory_grouped" in df.columns else ("category" if "category" in df.columns else None))
            if cat_col:
...
with colC:
    patt = st.text_input("Desen (glob)", "*.csv", help="Örn: sf_crime_*.csv", key="glob_list")
asc = (order == "Eski → Yeni")
    ...
    if st.button("🧠 Modeli Eğit (örnek)"):
        ...
        for q in quantiles:
            ...
            if q == 0.5:
                ...
                st.write("Pozitiflerde MAE (median):", float(mean_absolute_error(y_cnt.loc[te_idx_pos], pred_med)))
            q_models[q] = qr
        ...
        with tabs[1]:
            if 0.5 in q_models:
                ...
                expl_reg = shap.TreeExplainer(q_models[0.5])  # pozitiflerde eğitildiği için X_cnt üzerinden örnek al
