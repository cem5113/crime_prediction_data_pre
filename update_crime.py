...
# --------------------------
# Birleştir & zamanda özellikler  (YENİ BLOK)
# --------------------------

# df_old için garanti altına al
if "time" not in df_old.columns:
    df_old["time"] = "00:00:00"
if "date" in df_old.columns:
    df_old["date"] = pd.to_datetime(df_old["date"], errors="coerce").dt.date

# df_new varsa, time kolonunu garanti et
if not df_new.empty and ("time" not in df_new.columns):
...
# 5 yıllık pencereyi tarih üzerinden uygula
start_date_5y = today - timedelta(days=5*365)
df_all["date"] = pd.to_datetime(df_all["date"], errors="coerce")  # -> datetime64[ns]
df_all = df_all[df_all["date"].dt.date >= start_date_5y]

# --- KRİTİK DÜZELTME: datetime'ı tek yerde ve sağlam şekilde üret ---
# Saat kolonunu stringe çek ve boşları doldur
df_all["time"] = df_all["time"].astype(str).fillna("00:00:00")

# date + time -> datetime (NaT olanlar atılır), saat başına yuvarla
...
# Zaman dilimini sabitle: önce localize dene, olmazsa convert
try:
    ...
    try:
        ...
        # son çare: UTC'ye localize edip SF'e çevir
        try:
            df_all["datetime"] = df_all["datetime"].dt.tz_localize("UTC").dt.tz_convert(SF_TZ)
        except Exception:
            pass

# Türetilmiş anahtarlar
...
# Tatil hesapları (date_only -> normalize)
...
# Çıkış hedefini seç ve kaydet (aynı)
...
# crime_prediction_data/ kopyaları (aynı mantık)
try:
    Path("crime_prediction_data").mkdir(exist_ok=True)
